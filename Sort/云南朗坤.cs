...
public class 云南朗坤
{
    public static void Run(MemoryStream ExcelData, string excelFileName)
    {
        ...
        List<string> Uid16_Data = new List<string>();
        ...
        using (var package = new ExcelPackage())
        {
            ...
            for (var i = 0; i < SNData.Count; i++)
            {
                ...
                worksheet.Cells[i + 2, 3].Value = Uid16_Data[i];
                worksheet.Cells[i + 2, 4].Value = Convert.ToUInt32(Uid16Data[i], 16).ToString();
                worksheet.Cells[i + 2, 5].Value = Convert.ToUInt32(Uid16_Data[i], 16).ToString();
            }
